    ...
    public void setQuantidades(Map<String,IngredienteUtilizado> ingredientesReceita, Map<String,IngredienteUtilizado> ingredientesUsuarioReceita){
       List<Double> quantidadesTotais = Arrays.asList(0.0,0.0);
        for(Map.Entry<String,IngredienteUtilizado> entry : ingredientesReceita.entrySet()){
            ...
            if(ingredientesUsuarioReceita.containsKey(entry.getKey())) ingredienteUtilizadosTratamento.add(ingredientesUsuarioReceita.get(entry.getKey()));
            ...
            if(isMesmaClasseUnidade){
                ...
                if(quantidades.get(POSICAO_USUARIO) > quantidades.get(POSICAO_RECEITA)) quantidades.set(POSICAO_USUARIO, quantidades.get(POSICAO_RECEITA));
    ...
    public List<Double> tratarUnidades(List<IngredienteUtilizado> ingredienteUtilizadosTratamento, boolean isMesmaClasseUnidade){
        ...
        for(int i = 0; i <= limite; i++){
            ...
            switch (ingredienteUtilizado.getUnidade()) {
                case LITROS, QUILOS -> quantidades.add(ingredienteUtilizado.getQuantidade() * 1000);
                case MILILITROS, UNIDADES, GRAMAS -> quantidades.add(ingredienteUtilizado.getQuantidade());
                case COLHERES_DE_SOPA -> quantidades.add(ingredienteUtilizado.getQuantidade() * 15);
                case COLHERES_DE_CHA -> quantidades.add(ingredienteUtilizado.getQuantidade() * 5);
            }
        }
        return quantidades;
    }
